...
type DetailTier = keyof typeof DETAIL_STEPS;
const stepsFor = (detail: DetailTier) => DETAIL_STEPS[detail] || DETAIL_STEPS.medium;
...
const AcidSquares: React.FC<AcidSquaresProps> = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (rtA && rtB) {
        if (rtA.buffer) gl.deleteFramebuffer(rtA.buffer);
        if (rtB.buffer) gl.deleteFramebuffer(rtB.buffer);
